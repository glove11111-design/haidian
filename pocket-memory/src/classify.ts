...
const RULES: { name: string; keys: string[] }[] = [
  ...
  { name: '资料', keys: ['文章', '教程', '怎么', '方法', '笔记', '资料', '公众号', '分类'] },
...
export function classifyFromContent(input: {
  ...
  const hay = [input.text, input.caption, input.title, input.transcript]
    .filter(Boolean)
    .join('\n');
...
export function classifyItem(item: Pick<MemoryItem, 'type' | 'text' | 'caption' | 'title' | 'transcript' | 'videoKind' | 'sourceApp'>): string {
